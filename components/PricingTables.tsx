...
export function TattooPricingTable() {
  return (
    <div>
      <div className="grid gap-5 md:grid-cols-3">
        {tattooPricing.map((group) => (
          ...
          >
            ...
            <p className="mt-1 text-[12px] tracking-wide text-sand uppercase">
              {group.note}
            </p>
            {group.base && (
              ...
              {group.rates.map((rate) => (
                ...
                >
                  ...
                  <span className="font-semibold text-ink">
                    ...
                    <span className="text-xs text-sand">{rate.unit}</span>
                  </span>
                </li>
              ))}
            </ul>
      ...
      <p className="mt-6 text-center text-xs tracking-[0.2em] text-sand/70 uppercase">
        {tattooPricingNote}
      </p>
    </div>
  );
  ...
  return (
    <div className="mx-auto grid max-w-3xl gap-3 sm:grid-cols-2">
      {nailArtPricing.map((item) => (
        ...
        >
          <span className="text-[14px] text-ink/90">{item.service}</span>
